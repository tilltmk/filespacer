...
@click.option('--level', '-l', type=click.IntRange(1, 22), 
              help='Compression level (1-22, default from config or 3)')
@click.option('--exclude', '-e', multiple=True, 
              help='Patterns to exclude from folder compression')
...
def compress(ctx, input_path, output_path, level, exclude, no_hash, no_parallel, stats):
    ...
    try:
        if input_p.is_file():
            result = fs.compress_file(
                input_path, 
                output_path, 
                compression_level=level,
                calculate_hash=not no_hash
            )
        elif input_p.is_dir():
