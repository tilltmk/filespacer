...
class ModernFileSpacerGUI:
    ...
    def _create_main_ui(self):
        ...
        self._create_operation_button(
            button_frame, "Extract ZIP", self._show_extract_dialog,
            "Extract files from ZIP archives", 0
        )
        
        self._create_operation_button(
            button_frame, "Compress", self._show_compress_dialog,
            "Compress files or folders", 1
        )
        
        self._create_operation_button(
            button_frame, "Decompress", self._show_decompress_dialog,
            "Decompress ZST files", 2
        )
...
class CompressDialog(BaseDialog):
    ...
    def _create_ui(self):
        ...
        level_scale = ttk.Scale(
            level_frame, from_=1, to=22, variable=self.level_var,
...
class DecompressDialog(BaseDialog):
    ...
    def decompress(self):
        ...
        self.run_operation(
            self.filespacer.extract_zst,
            self.input_var.get(),
            self.output_var.get(),
            verify_hash=self.verify_var.get()
        )
